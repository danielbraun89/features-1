...
# npm version for cdktf-cli
TERRAFORM_CDK=${VERSION:-"latest"}
...
if [ "$TERRAFORM_CDK" != "none" ]; then
    if [ "$TERRAFORM_CDK" =  "latest" ]; then
        util_command="cdktf-cli"
    else
        util_command="cdktf-cli@$TERRAFORM_CDK"
    fi
